...
def main():
    ...
    if args.do_train:
        ...
        feature_for_predict = ["F1"]

        train = preprocess_samples(train_set, feature_for_training, feature_for_predict)
        valid = preprocess_samples(valid_set, feature_for_training, feature_for_predict)
        ...
        trainer = Trainer(trainData, validData, hidden_size=args.hidden_size, device=device, model_dir=args.arch)
    ...
    if args.do_predict:
        ...
        model.load_state_dict(torch.load('%s/model.pkl.%d' % (args.arch, args.ckpt))['classficationA'])
        model.train(False)
        model.to(device)
