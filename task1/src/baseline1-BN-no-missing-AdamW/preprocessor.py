...
def preprocess_sample(data, feature_for_training=None, feature_for_prediction=None):
    ...
    """
    processed = {}
    if feature_for_training is not None:
        processed['Features'] = [data[feature] for feature in feature_for_training]
    if feature_for_prediction is not None:
        processed['Label_features'] = [data[feature] for feature in feature_for_prediction]
    if 'Class' in data:
